...
parseFloat(d.lat),g=parseFloat(d.lon);d=parseInt(d.zoom);if(d<9)d+=3;if(d>9)d=9;f.innerHTML="loading...";map.center({lat:e,lon:g});map.zoom(d);setTimeout(c,2500)},function(d){f.innerHTML="your query failed with the following error: "+d.message;setTimeout(c,7500)});f.innerHTML="searching..."}}document.onkeydown=function(a){if(a.keyCode==27){a=document.getElementById("footer");var f=a.style.display;if(f=="")f="none";a.style.display=f=="none"?"block":"none"}};function tilestache(a){function f(b){b=String(b);var c=6-b.length;b=c<1?b:Array(c+1).join("0")+b;return b.substr(0,3)+"/"+b.substr(3)}return function(b){var c=1<<b.zoom,d=b.column%c;if(d<0)d+=c;return a.replace(/{(.)}/g,function(e,g){switch(g){case "Z":return b.zoom;case "X":return f(d);case "Y":return f(b.row)}return g})}};function load_staticmap(){zoomable_places.push([45.413479,-73.976608,45.704788,-73.476418]);zoomable_places.push([38.673569,-9.24054,38.795689,-9.04763]);var a=pm.svg("svg");a=document.body.appendChild(a);var f=pm.image();f.url(tilestache(tile_server+"/isola/{Z}/{X}/{Y}.png"));var b=pm.interact(),c=pm.hash();map=pm.map();map.container(a);map.center({lat:19.366349,lon:-103.083425});map.zoomRange([2,10]);map.zoom(2);map.add(b);map.add(c);map.add(f);map.on("move",function(){var d=map.zoom(),e=map.zoomRange();
d=e[1];e=map.center();if(is_zoomable(e.lat,e.lon))d!=15&&map.zoomRange([2,15]);else d!=10&&map.zoomRange([2,10]);d=map.zoom();e=map.zoomRange();var g=document.getElementById("near"),h=document.getElementById("far"),j=d<=e[0]?"none":"block";g.style.display=d>=e[1]?"none":"block";h.style.display=j})}function toggle_static_notice(a){document.getElementById("static").style.display=a?"block":"none"}
function goto_prettymaps(){var a=map.center();a="/201008/#"+map.zoom()+"/"+a.lat+"/"+a.lon;location.href=a};
